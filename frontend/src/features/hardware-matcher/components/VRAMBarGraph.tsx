  ...
  return (
    <Card className="p-6">
      <div className="flex justify-between mb-2">
        <h3 className="text-lg font-semibold text-slate-800">VRAM Usage</h3>
        <span className={`font-semibold ${isOOM ? 'text-red-500' : 'text-slate-600'}`}>
          {result.status.replace(/_/g, ' ')}
        </span>
      </div>

      <div className="relative h-8 bg-slate-200 rounded-lg overflow-hidden flex">
        <div 
